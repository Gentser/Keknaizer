    ...
    ui(new Ui::MainWindow)
{
    ...
    //    }
        ...
        try{
            ...
            Timeline<std::string> *second = new Timeline<std::string>("Sec",
                                                                     QDateTime(QDate(2001,1,1), QTime(0,0)),
                                                                     QDateTime(QDate(2020,2,3), QTime(15,35)) );

            second->addItem(new TimelineItem<std::string>(QDateTime(QDate(2015,2,20), QTime(17,59)),
                                                          QDateTime(QDate(2015,10,31), QTime(2,20)),
                                                          std::string("KIO task!")));

            second->addItem(new TimelineItem<std::string>(QDateTime(QDate(2010,4,20), QTime(17,59)),
                                                          QDateTime(QDate(2017,10,31), QTime(2,20)),
                                                          std::string("ZAZAZA")));

            Diagram->addTimeline(second);
